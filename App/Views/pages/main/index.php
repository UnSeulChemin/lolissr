...
if ($stats === null) {
    throw new \RuntimeException('Stats manquantes dans la vue.');
}

$baseUri = rtrim((string)($baseUri ?? ''), '/') . '/';
$hasLongestSeries = $stats->longestSeries !== null;
$hasLastTome = $stats->lastTome !== null;
$hasTopLongestSeries = is_iterable($stats->topLongestSeries) && count((array)$stats->topLongestSeries) > 0;
?>

<section class="layout-container">

    <!-- Top Grid: Longest Series & Last Tome -->
    <section class="home-grid home-grid-top card-grid-3 animate-fade-up-stagger">
        ...
        <?php if ($hasLongestSeries): ?>
            ...
            $href = $baseUri . 'manga/series/' . rawurlencode((string)$serie->slug);
            $thumbnailPath = $baseUri . 'images/mangas/thumbnail/' . $serie->thumbnail . '.' . $serie->extension;
            ?>
            <a class="card card-link card-link-wide card-wide" href="<?= e($href) ?>">
                ...
                <div class="home-longest-content">
                    <div class="card-image-box-portrait home-feature-image-box">
                        <img class="card-image-portrait" src="<?= e($thumbnailPath) ?>" alt="<?= e($serie->livre) ?>">
                    </div>
                    <div class="home-longest-info">
                        <p class="home-longest-name"><?= e($serie->livre) ?></p>
                        <p class="home-longest-count"><?= (int)$serie->total ?> tomes</p>
                    </div>
                </div>
            </a>
        <?php else: ?>
        ...
        <?php if ($hasLastTome): ?>
            <?php
            $tome = $stats->lastTome;
            $href = $baseUri . 'manga/series/' . rawurlencode((string)$tome->slug) . '/' . (int)$tome->numero;
            $thumbnailPath = $baseUri . 'images/mangas/thumbnail/' . $tome->thumbnail . '.' . $tome->extension;
            ?>
            <a class="card card-link card-medium" href="<?= e($href) ?>">
                <h2 class="home-card-title">🆕 Dernier tome ajouté</h2>
                <div class="home-feature-content">
                    <div class="card-image-box-portrait home-feature-image-box">
                        <img class="card-image-portrait" src="<?= e($thumbnailPath) ?>" alt="<?= e($tome->livre) ?>">
                    </div>
                    <div class="home-feature-info">
                        <p class="home-feature-title"><?= e($tome->livre) ?></p>
                        <p class="home-feature-meta">Tome <?= str_pad((string)$tome->numero, 2, '0', STR_PAD_LEFT) ?></p>
                    </div>
                </div>
            </a>
        <?php else: ?>
    ...
    <?php if ($hasTopLongestSeries): ?>
        ...
        <section class="home-ranking-list card-list animate-fade-up-stagger">
            <?php foreach ($stats->topLongestSeries as $index => $serie): ?>
                ...
                $thumbnailPath = $baseUri . 'images/mangas/thumbnail/' . $serie->thumbnail . '.' . $serie->extension;
                ?>
                <a class="card card-link card-bottom" href="<?= e($href) ?>">
                    <p class="home-series-rank">#<?= $index + 1 ?></p>
                    <div class="card-image-box-portrait">
                        <img class="card-image-portrait" src="<?= e($thumbnailPath) ?>" alt="<?= e($serie->livre) ?>">
                    </div>
                    <p class="home-list-card-title"><?= e($serie->livre) ?></p>
                    <p class="home-list-card-meta"><?= (int)$serie->total ?> tomes</p>
                </a>
            <?php endforeach; ?>
        </section>
    <?php endif; ?>
    ...
    <section class="home-grid home-grid-stats card-grid-3 animate-fade-up-stagger">
        ...
        <article class="card card-small home-reading-progress-card">
            ...
            <p class="home-card-value home-reading-progress-value"><?= (int)$stats->readingProgress ?>%</p>
            <div class="home-reading-progress" style="--progress: <?= (int)$stats->readingProgress ?>%;">
