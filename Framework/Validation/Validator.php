...
final class Validator
{
    ...
    public function numeric(string $field, ?string $message = null): self
    {
        ...
        $value = $this->floatValue($field) ?? $this->integerValue($field);
    ...
    private function integerValue(string $field): ?int
    {
        ...
        return $this->integerCache[$field] = filter_var($value, FILTER_VALIDATE_INT) !== false ? (int) $value: null;
    }

    private function floatValue(string $field): ?float
    {
        if (array_key_exists($field, $this->floatCache))
        {
        ...
        return $this->floatCache[$field] = filter_var($value, FILTER_VALIDATE_FLOAT) !== false ? (float) $value : null;
    }
    ...
    public function in(string $field, array $allowedValues, ?string $message = null): self
    {
        ...
        if (! in_array($this->data[$field] ?? null, $allowedValues, true))
        {
    ...
    {
        return isset($this->files[$field]['tmp_name'])
            && is_string($this->files[$field]['tmp_name'])
            && $this->files[$field]['tmp_name'] !== '';
    }

    private function shouldSkipFile(string $field): bool
    {
        return $this->shouldSkip($field) || ! $this->hasUploadedFile($field);
    }
    ...
    public function fileOk(string $field, ?string $message = null): self
    {
        ...
        if ($this->hasError($field) || ! $this->hasUploadedFile($field))
        {
            return $this;
        }

        if (($this->files[$field]['error'] ?? null) !== UPLOAD_ERR_OK)
        {
    ...
    public function imageMime(string $field, array $allowedMimeTypes, ?string $message = null): self
    {
        ...
        if (! is_string($mimeType) || ! in_array(strtolower($mimeType), $allowedMimeTypes, true))
        {
