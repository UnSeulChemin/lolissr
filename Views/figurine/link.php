<section class="section-content">

    <h2>Figurine Link</h2>

    <section class="flex-column-gap-30">

        <div class="link-div">
            <p class="table-colonne m-b-20">AmiAmi</p>
            <p class="link-p">Site d'achat</p>
            <a class="link-section" href="https://www.amiami.com/eng/search/list/?s_st_list_preorder_available=1&s_st_list_backorder_available=1&s_st_list_newitem_available=1&s_st_condition_flg=1&s_keywords=complete+figure"
                target="_blank">Lien</a>
        </div>

        <div class="link-div">
            <p class="table-colonne m-b-20">MyFigureCollection</p>
            <p class="link-p">Site d'information</p>
            <a class="link-section" href="https://myfigurecollection.net" target="_blank">Lien</a>
        </div>

    </section>

    <div class="m-t-30">
        <a class="link-section" href="javascript:history.go(-1)">Back</a>
    </div>

</section>